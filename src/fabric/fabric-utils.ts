...
export const initializeFabric = async (canvasEl, containerEl) => {
  try {
    ...
    // Set default canvas dimensions (1080x1080 for social media posts)
...
export const centerCanvas = (canvas) => {
  if (!canvas || !canvas.wrapperEl) {
    console.warn("centerCanvas: canvas or wrapperEl not available");
    return;
  }

  console.log("centerCanvas: centering with dimensions", { 
    width: canvas.width, 
  ...
  canvasWrapper.style.minWidth = `${canvas.width}px`;
  canvasWrapper.style.minHeight = `${canvas.height}px`;
  canvasWrapper.style.maxWidth = `${canvas.width}px`;
  canvasWrapper.style.maxHeight = `${canvas.height}px`;
